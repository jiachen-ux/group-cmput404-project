...
function delete_post(id) {
    ...
    setTimeout(() => {
        ...
        fetch('service/authors/post/'+parseInt(id)+'/delete', {
            method: 'PUT'
        });
    },200);
}
...
function edit_post_submit(post_id) {
    ...
    fetch('service/authors/post/'+parseInt(post_id)+'/edit', {
        method:'POST',
        body: formdata
    })
...
function like_post(element) {
    ...
    fetch('service/authors/post/'+parseInt(id)+'/like', {
        method: 'PUT'
    })
    .then(() => {
...
function unlike_post(element) {
    let id = element.dataset.post_id;
    fetch('service/authors/post/'+parseInt(id)+'/unlike', {
        method: 'PUT'
    })
    .then(() => {
        let count = element.querySelector('.likes_count');
...
function follow_user(element, username, origin) {
    ...
    fetch('service/authors/'+username+'/follow', {
        method: 'PUT'
    })
    .then(() => {
...
function unfollow_user(element, user, origin) {
    ...
    fetch('service/authors/'+username+'/unfollow', {
        method: 'PUT'
    })
    .then(() => {
        if(origin === 'suggestion') {
...
function show_comment(element) {
    ...
    fetch('service/authors/post/'+parseInt(post_id)+'/comments')
    .then(response => response.json())
    .then(comments => {
...
function write_comment(element) {
    ...
    fetch('service/authors/post/'+parseInt(post_id)+'/write_comment',{
        method: 'POST',
        body: JSON.stringify({
...
function goto_register() {
    window.location.href = 'service/authors/register';
}

function goto_login() {
    window.location.href = 'service/authors/login';
}
